...
public:
    ...
    void generate_bad_examples(unsigned int count, deque<IndexedExample*>& examples);

    void generate_good_examples_flow(deque<IndexedExample*>& examples, unsigned int count);
...
private:
    ...
    void eval_rhs(Expr* rhs, IndexedExample* eg, unsigned int time, metric_val& res) const;
